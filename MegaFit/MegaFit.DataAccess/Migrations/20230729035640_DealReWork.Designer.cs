// <auto-generated />
using System;
using MegaFit.DataAccess;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace MegaFit.DataAccess.Migrations
{
    [DbContext(typeof(MegaContext))]
    [Migration("20230729035640_DealReWork")]
    partial class DealReWork
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.15")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.Appointment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<DateTime>("AppointmentDate")
                        .HasColumnType("datetime2(0)");

                    b.Property<int>("DealId")
                        .HasColumnType("int");

                    b.Property<bool>("IsCompleted")
                        .HasColumnType("bit");

                    b.HasKey("Id");

                    b.HasIndex("DealId");

                    b.ToTable("Appointment", (string)null);
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.AppointmentTime", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<short>("AppointmentHour")
                        .HasColumnType("smallint");

                    b.HasKey("Id");

                    b.ToTable("AppointmentTime", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            AppointmentHour = (short)9
                        },
                        new
                        {
                            Id = 2,
                            AppointmentHour = (short)10
                        },
                        new
                        {
                            Id = 3,
                            AppointmentHour = (short)11
                        },
                        new
                        {
                            Id = 4,
                            AppointmentHour = (short)12
                        },
                        new
                        {
                            Id = 5,
                            AppointmentHour = (short)13
                        },
                        new
                        {
                            Id = 6,
                            AppointmentHour = (short)14
                        },
                        new
                        {
                            Id = 7,
                            AppointmentHour = (short)15
                        },
                        new
                        {
                            Id = 8,
                            AppointmentHour = (short)16
                        },
                        new
                        {
                            Id = 9,
                            AppointmentHour = (short)17
                        },
                        new
                        {
                            Id = 10,
                            AppointmentHour = (short)18
                        },
                        new
                        {
                            Id = 11,
                            AppointmentHour = (short)19
                        },
                        new
                        {
                            Id = 12,
                            AppointmentHour = (short)20
                        });
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.Customer", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("Age")
                        .HasColumnType("int");

                    b.Property<string>("CustomerAddress")
                        .HasColumnType("varchar(250)");

                    b.Property<string>("EmailAddress")
                        .HasColumnType("varchar(30)");

                    b.Property<string>("FirstName")
                        .IsRequired()
                        .HasColumnType("varchar(30)");

                    b.Property<int>("Height")
                        .HasColumnType("int");

                    b.Property<string>("IdentityNumber")
                        .IsRequired()
                        .HasColumnType("varchar(20)");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasColumnType("varchar(30)");

                    b.Property<string>("PhoneNumber")
                        .IsRequired()
                        .HasColumnType("varchar(20)");

                    b.Property<int>("Weight")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Customer", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Age = 0,
                            CustomerAddress = "Uskudar",
                            EmailAddress = "mustafakorkmmaz@gmail.com",
                            FirstName = "Mustafa",
                            Height = 0,
                            IdentityNumber = "11102613645",
                            LastName = "Korkmaz",
                            PhoneNumber = "05343494123",
                            Weight = 0
                        },
                        new
                        {
                            Id = 2,
                            Age = 0,
                            CustomerAddress = "Kartal",
                            EmailAddress = "mesutrizatunur@gmail.com",
                            FirstName = "Mesut Rıza",
                            Height = 0,
                            IdentityNumber = "313569752136",
                            LastName = "Tunur",
                            PhoneNumber = "05343494122",
                            Weight = 0
                        },
                        new
                        {
                            Id = 3,
                            Age = 0,
                            CustomerAddress = "Maltepe",
                            EmailAddress = "yunusakbulut@gmail.com",
                            FirstName = "Yunus",
                            Height = 0,
                            IdentityNumber = "22269813645",
                            LastName = "Akbulut",
                            PhoneNumber = "05343494121",
                            Weight = 0
                        });
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.CustomerAssessment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Evaluation")
                        .IsRequired()
                        .HasColumnType("varchar(max)");

                    b.Property<int>("VisitId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("VisitId");

                    b.ToTable("CustomerAssessment", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Evaluation = "Deneme",
                            VisitId = 1
                        },
                        new
                        {
                            Id = 2,
                            Evaluation = "Deneme",
                            VisitId = 2
                        },
                        new
                        {
                            Id = 3,
                            Evaluation = "Deneme",
                            VisitId = 3
                        });
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.CustomerVisit", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Complaint")
                        .IsRequired()
                        .HasColumnType("varchar(1000)");

                    b.Property<int>("CustomerId")
                        .HasColumnType("int");

                    b.Property<bool>("IsSick")
                        .HasColumnType("bit");

                    b.Property<string>("SicknessList")
                        .HasColumnType("varchar(250)");

                    b.Property<bool>("UsingDrugs")
                        .HasColumnType("bit");

                    b.Property<DateTime>("VisitDate")
                        .HasColumnType("datetime2(0)");

                    b.HasKey("Id");

                    b.HasIndex("CustomerId");

                    b.ToTable("CustomerVisit", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Complaint = "Şikayet",
                            CustomerId = 1,
                            IsSick = true,
                            UsingDrugs = true,
                            VisitDate = new DateTime(2023, 7, 29, 6, 56, 40, 397, DateTimeKind.Local).AddTicks(6671)
                        },
                        new
                        {
                            Id = 2,
                            Complaint = "Test",
                            CustomerId = 2,
                            IsSick = false,
                            UsingDrugs = false,
                            VisitDate = new DateTime(2023, 7, 29, 6, 56, 40, 399, DateTimeKind.Local).AddTicks(145)
                        },
                        new
                        {
                            Id = 3,
                            Complaint = "Deneme",
                            CustomerId = 3,
                            IsSick = true,
                            UsingDrugs = false,
                            VisitDate = new DateTime(2023, 7, 29, 6, 56, 40, 399, DateTimeKind.Local).AddTicks(151)
                        });
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.Deal", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("AppointmentDayFirst")
                        .IsRequired()
                        .HasColumnType("varchar(20)");

                    b.Property<string>("AppointmentDaySecond")
                        .IsRequired()
                        .HasColumnType("varchar(20)");

                    b.Property<int>("AppointmentTimeId")
                        .HasColumnType("int");

                    b.Property<int>("CustomerId")
                        .HasColumnType("int");

                    b.Property<int>("PackageId")
                        .HasColumnType("int");

                    b.Property<int>("TotalSession")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("AppointmentTimeId");

                    b.HasIndex("CustomerId");

                    b.HasIndex("PackageId");

                    b.ToTable("Deal", (string)null);
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.Package", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("MonthCount")
                        .HasColumnType("int");

                    b.Property<int>("TherapyId")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.HasIndex("TherapyId");

                    b.ToTable("Package", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            MonthCount = 1,
                            TherapyId = 1
                        },
                        new
                        {
                            Id = 2,
                            MonthCount = 2,
                            TherapyId = 1
                        },
                        new
                        {
                            Id = 3,
                            MonthCount = 3,
                            TherapyId = 1
                        },
                        new
                        {
                            Id = 4,
                            MonthCount = 1,
                            TherapyId = 2
                        },
                        new
                        {
                            Id = 5,
                            MonthCount = 2,
                            TherapyId = 2
                        },
                        new
                        {
                            Id = 6,
                            MonthCount = 3,
                            TherapyId = 2
                        },
                        new
                        {
                            Id = 7,
                            MonthCount = 1,
                            TherapyId = 3
                        },
                        new
                        {
                            Id = 8,
                            MonthCount = 2,
                            TherapyId = 3
                        },
                        new
                        {
                            Id = 9,
                            MonthCount = 3,
                            TherapyId = 3
                        });
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.Therapy", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasColumnType("varchar(20)");

                    b.HasKey("Id");

                    b.ToTable("Therapy", (string)null);

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Name = "GTOS"
                        },
                        new
                        {
                            Id = 2,
                            Name = "Pilates"
                        },
                        new
                        {
                            Id = 3,
                            Name = "Fizik Tedavi"
                        });
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.Appointment", b =>
                {
                    b.HasOne("MegaFit.EntityLayer.Entities.Deal", "Deal")
                        .WithMany()
                        .HasForeignKey("DealId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Deal");
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.CustomerAssessment", b =>
                {
                    b.HasOne("MegaFit.EntityLayer.Entities.CustomerVisit", "CustomerVisit")
                        .WithMany()
                        .HasForeignKey("VisitId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("CustomerVisit");
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.CustomerVisit", b =>
                {
                    b.HasOne("MegaFit.EntityLayer.Entities.Customer", "Customer")
                        .WithMany()
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Customer");
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.Deal", b =>
                {
                    b.HasOne("MegaFit.EntityLayer.Entities.AppointmentTime", "AppointmentTime")
                        .WithMany()
                        .HasForeignKey("AppointmentTimeId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("MegaFit.EntityLayer.Entities.CustomerAssessment", "CustomerAssessment")
                        .WithMany()
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("MegaFit.EntityLayer.Entities.Package", "Package")
                        .WithMany()
                        .HasForeignKey("PackageId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("AppointmentTime");

                    b.Navigation("CustomerAssessment");

                    b.Navigation("Package");
                });

            modelBuilder.Entity("MegaFit.EntityLayer.Entities.Package", b =>
                {
                    b.HasOne("MegaFit.EntityLayer.Entities.Therapy", "Therapy")
                        .WithMany()
                        .HasForeignKey("TherapyId")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.Navigation("Therapy");
                });
#pragma warning restore 612, 618
        }
    }
}
